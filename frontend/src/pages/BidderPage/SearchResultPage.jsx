    ...
    const fetchAuctions = async () => {
        ...
        try {
            const params = {
                page: filters.page,
                limit: filters.limit,
                title: searchQuery // Pass search query to API
            };
            ...
            // Optional: Client-side filtering if API doesn't support title search yet
            let fetchedAuctions = response.data?.data?.auctions || [];
            if (searchQuery && !response.config.params.title) {
                // If we suspect API ignores title param, we can doubly ensure here, 
                // but typically we trust the API or let it be. 
                // Let's assume API handles it or returns all.
                // For better UX if API is dumb:
                fetchedAuctions = fetchedAuctions.filter(a => a.title.toLowerCase().includes(searchQuery.toLowerCase()));
            }
        ...
        <div className="w-full min-h-screen bg-[#f8f7f5] text-[#1c130d] px-4 md:px-8 py-6">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4 text-sm max-w-[1920px] mx-auto">
                ...
                {searchQuery && (
                    <>
                        <span className="text-gray-400">/</span>
                        <span className="text-orange-600 font-medium">"{searchQuery}"</span>
                    </>
                )}
            </div>
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 max-w-[1920px] mx-auto">
                <div className="flex flex-col gap-2">
                    <h1 className="text-3xl md:text-4xl font-black leading-tight tracking-tight text-gray-900">
                        {searchQuery ? `Kết quả tìm kiếm cho "${searchQuery}"` : 'Tất cả kết quả'}
                    </h1>
                    <p className="text-gray-500 text-base">
                        Tìm thấy <span className="font-bold text-orange-600">{totalAuctions}</span> kết quả phù hợp.
                    </p>
                </div>
            </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                        <p className="text-sm font-medium text-gray-700">
                            Hiển thị <span className="font-bold">{auctions.length}</span> kết quả
                        </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                        {loading ? (
                            ...
                            <div className="col-span-full py-20 bg-white rounded-xl border border-gray-200 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="mt-4 text-gray-500 font-medium">Không tìm thấy sản phẩm nào phù hợp với "{searchQuery}"</p>
                                <button
                                    onClick={() => setFilters({ status: ['active', 'pending'], category: 'all', minPrice: 0, maxPrice: 1000000000, sortBy: 'ending_soon', page: 1, limit: 12 })}
                    ...
                    {totalAuctions > filters.limit && (
                        <div className="mt-12 flex items-center justify-center gap-2">
                            ...
                            <span className="text-gray-600 font-bold">Trang {filters.page}</span>
